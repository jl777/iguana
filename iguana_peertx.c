...
#define RAMCHAIN_ARGS struct iguana_ramchain *ramchain,struct iguana_txid *T,struct iguana_unspent *U,struct iguana_Uextra *U2,struct iguana_spend256 *S,struct iguana_pkhash *P,struct iguana_pkextra *P2,struct iguana_account *A,bits256 *X
#define RAMCHAIN_PTRPS struct iguana_ramchain *ramchain,struct iguana_txid **T,struct iguana_unspent **U,struct iguana_Uextra **U2,struct iguana_spend256 **S,struct iguana_pkhash **P,struct iguana_pkextra **P2,struct iguana_account **A,bits256 **X
#define RAMCHAIN_ARG ramchain,T,U,U2,S,P,P2,A,X
#define RAMCHAIN_PTRS struct iguana_txid *T; struct iguana_unspent *U; struct iguana_Uextra *U2; struct iguana_spend256 *S; struct iguana_pkhash *P; struct iguana_pkextra *P2; struct iguana_account *A; bits256 *X

uint32_t iguana_ramchain_addtxid(struct iguana_info *coin,RAMCHAIN_ARGS,bits256 txid,int32_t numvouts,int32_t numvins)
{
    struct iguana_kvitem *ptr; uint32_t txidind; struct iguana_txid *t;
...
uint32_t iguana_ramchain_addpkhash(struct iguana_info *coin,RAMCHAIN_ARGS,uint8_t rmd160[20],int32_t flags,uint32_t unspentind)
{
    struct iguana_kvitem *ptr; int32_t pkind = -1;
...
uint32_t iguana_ramchain_addunspent(struct iguana_info *coin,RAMCHAIN_ARGS,uint64_t value,uint8_t *script,int32_t scriptlen,bits256 txid,int32_t vout)
{
    //struct iguana_unspent { uint64_t value; uint32_t txidind,pkind,prevunspentind; } __attribute__((packed));
    uint8_t rmd160[20]; struct iguana_kvitem *ptr; uint32_t unspentind; int32_t pkind; struct iguana_unspent *u;
    ...
    if ( ramchain->ROflag != 0 )
    {
        ...
        if ( u->pkind != pkind || u->value != value || u->txidind != ramchain->txidind )//|| u->prevunspentind != A[pkind].lastunspentind )
        {
    ...
    {
        u->value = value, u->txidind = ramchain->txidind, u->pkind = pkind;
...
uint32_t iguana_ramchain_addspend256(struct iguana_info *coin,RAMCHAIN_ARGS,bits256 prev_hash,int32_t prev_vout,uint8_t *script,int32_t scriptlen,uint32_t sequence,int32_t hdrsi)
{
    ...
    if ( (external= ((txidind >> 31) & 1)) == 0 )
    {
        ...
        if ( (pkind= U[unspentind].pkind) >= ramchain->data->numpkinds )
        {
...
int64_t iguana_ramchain_init(struct iguana_ramchain *ramchain,struct iguana_memspace *mem,struct iguana_memspace *hashmem,int32_t firsti,int32_t numtxids,int32_t numunspents,int32_t numspends,int32_t numpkinds,int32_t numexternaltxids)
{
    ...
    ramchain->data->Uoffset = offset, offset += (sizeof(struct iguana_unspent) * numunspents);
...
long iguana_ramchain_save(struct iguana_info *coin,RAMCHAIN_ARGS,uint32_t ipbits,bits256 hash2,int32_t bundlei)
{
    ...
    if ( fp != 0 )
    {
        ...
        ramchain->data->Uoffset = offset, offset += (sizeof(struct iguana_unspent) * rdata->numunspents);
        ...
        *rdata = tmp;
        ...
        fwrite(U,sizeof(struct iguana_unspent),rdata->numunspents,fp);
...
/*int32_t iguana_ramchain_txid(struct iguana_info *coin,RAMCHAIN_ARGS,bits256 *txidp,struct iguana_spend256 *s)
{
...
{
    RAMCHAIN_PTRS; struct iguana_txid *t; struct iguana_unspent *u; struct iguana_spend256 *s;
    struct iguana_pkhash *p; struct iguana_ramchaindata *rdata;
    int32_t k,pkind; struct iguana_kvitem *ptr;
    ...
    for (ramchain->txidind=rdata->firsti; ramchain->txidind<rdata->numtxids; ramchain->txidind++)
    {
        ...
        for (k=0; k<t->numvins; k++,ramchain->spendind++)
        {
            ...
            /*if ( (vout= iguana_ramchain_txid(coin,RAMCHAIN_ARG,&txid,s)) < -1 )
            {
                printf("txidind.%d k.%d error getting txid firsti.%d X.%d vout.%d spend.%x/%d numX.%d numT.%d\n",ramchain->txidind,k,rdata->firsti,ramchain->externalind,vout,s->spendtxidind,rdata->numspends,rdata->numexternaltxids,rdata->numtxids);
                return(-10);
            }
            if ( vout == -1 )
            {
...
struct iguana_ramchain *iguana_ramchain_map(struct iguana_info *coin,struct iguana_ramchain *ramchain,struct iguana_memspace *hashmem,uint32_t ipbits,bits256 hash2,int32_t bundlei,long fpos,int32_t allocextras)
{
    ...
    if ( (ptr= map_file(fname,&filesize,0)) != 0 )
    {
        ...
        {
            printf("iguana_ramchain_map.(%s) unexpected numblocks.%d\n",fname,ramchain->data->numblocks);
            munmap(ptr,filesize);
        }
        else
        {
            //printf("iguana_ramchain_map.(%s) size %ld vs %ld\n",fname,(long)iguana_ramchain_size(ramchain),(long)ramchain->data->allocsize);
            //iguana_meminit(&ramchain->MEM,"mapramchain",ramchain->data,ramchain->data->allocsize,0);
            //ramchain->txidind = ramchain->unspentind = ramchain->spendind = ramchain->pkind = ramchain->data->firsti;
            //ramchain->externalind = 0;
            if ( allocextras != 0 )
...
{
    int32_t i; struct iguana_txid *Ta,*Tb; struct iguana_unspent *Ua,*Ub; struct iguana_spend256 *Sa,*Sb; struct iguana_pkhash *Pa,*Pb; bits256 *Xa,*Xb; struct iguana_Uextra *U2a,*U2b; struct iguana_pkextra *P2a,*P2b; struct iguana_account *ACCTa,*ACCTb; char str[65],str2[65];
    if ( A->data != 0 && B->data != 0 && A->data->numblocks == B->data->numblocks && memcmp(A->data->firsthash2.bytes,B->data->firsthash2.bytes,sizeof(A->data->firsthash2)) == 0 )
    {
        ...
        {
            _iguana_ramchain_setptrs(A,&Ta,&Ua,&U2a,&Sa,&Pa,&P2a,&ACCTa,&Xa);
            _iguana_ramchain_setptrs(B,&Tb,&Ub,&U2b,&Sb,&Pb,&P2b,&ACCTb,&Xb);
            ...
            for (i=0; i<A->data->numexternaltxids; i++)
                ...
                {
                    //for (j=0; j<A->data->numexternaltxids; j++)
                    {
...
{
    RAMCHAIN_PTRS; int32_t j;
    struct iguana_txid *tx; struct iguana_spend256 *s; struct iguana_ramchaindata *rdata;
    _iguana_ramchain_setptrs(ramchain,&T,&U,&U2,&S,&P,&P2,&A,&X);
    if ( (rdata= ramchain->data) == 0 )
        return(-1);
    ramchain->ROflag = 1;
    ...
    for (ramchain->txidind=rdata->firsti; ramchain->txidind<rdata->numtxids; ramchain->txidind++)
    {
        ...
        {
            s = &S[j];
            //prevout = iguana_ramchain_txid(coin,RAMCHAIN_ARG,&prevhash,s);
            //sequence = (s->diffsequence == 0) ? 0xffffffff : 0;
            //char str[65]; printf("vin.%d %s vout.%d\n",j,bits256_str(str,s->prevhash2),s->prevout);
        }
    }
    return(0);
}
...
{
    RAMCHAIN_PTRS; long fsize; void *ptr; struct iguana_ramchain R,*mapchain,*ramchain = &addr->ramchain;
    ...
    for (i=0; i<txn_count; i++,ramchain->txidind++)
    {
        ...
        {
            iguana_ramchain_addunspent(coin,RAMCHAIN_ARG,tx->vouts[j].value,tx->vouts[j].pk_script,tx->vouts[j].pk_scriptlen,tx->txid,j);
        }

...
int32_t iguana_sendblockreq(struct iguana_info *coin,struct iguana_peer *addr,struct iguana_bundle *bp,int32_t bundlei,bits256 *hashes,int32_t n)
{
    int32_t len,allocsize; uint8_t *serialized;
    allocsize = sizeof(struct iguana_msghdr) + 32 + n * (sizeof(uint32_t) + sizeof(bits256));
    serialized = mycalloc('s',1,allocsize);
    if ( (len= iguana_getdata(coin,serialized,MSG_BLOCK,hashes,n)) > 0 )
    {
        ...
        {
            //char str[65]; printf("REQ.%s (%d:%d) n.%d\n",bits256_str(str,hash2),bp->hdrsi,bundlei,n);
            bp->issued[bundlei] = (uint32_t)time(NULL);
            if ( bp->requests[bundlei] < 100 )
                bp->requests[bundlei]++;
        }
    } else printf("MSG_BLOCK null datalen.%d\n",len);
    myfree(serialized,allocsize);
    return(len);
}

int32_t iguana_sendtxidreq(struct iguana_info *coin,struct iguana_peer *addr,bits256 hash2)
{
    uint8_t serialized[sizeof(struct iguana_msghdr) + sizeof(uint32_t)*32 + sizeof(bits256)]; int32_t len,i,r,j;
...
void iguana_gotblockM(struct iguana_info *coin,struct iguana_peer *addr,struct iguana_txblock *origtxdata,struct iguana_msgtx *txarray,uint8_t *data,int32_t recvlen)
{
    ...
    {
        for (i=0; i<origtxdata->block.txn_count; i++)
        {
    ...
    if ( bits256_nonz(bp->allhash) > 0 && num >= coin->chain->bundlesize )
    {
        ...
        if ( memcmp(allhash.bytes,bp->allhash.bytes,sizeof(allhash)) == 0 )
        {
            for (i=n=0; i<coin->chain->bundlesize; i++)
            {
                ...
                if ( (block= iguana_blockfind(coin,blockhashes[i])) == 0 || bp->fpos[i] < 0 )
                {
                    if ( block != 0 && block->copyflag != 0 )
                        printf("have data %d\n",bp->bundleheight+i);
                    else if ( strcmp(coin->symbol,"BTC") != 0 && bp->requests[i] < 3 )
                    {
    ...
    *blockp = block;
    if ( block != 0 )
    {
            ...
            if ( bundlei < coin->chain->bundlesize )
            {
                ...
                if ( bundlei == 0 )
                {
                    if ( bp->hdrsi > 0 && (bp= coin->bundles[bp->hdrsi-1]) != 0 )
                    {
                        ...
                        if ( bp->fpos[coin->chain->bundlesize-1] < 0 && strcmp(coin->symbol,"BTC") != 0 )
                ...
                else
                {
                    ...
                    if ( bp->fpos[bundlei-1] < 0 && strcmp(coin->symbol,"BTC") != 0 )
        ...
        if ( (bp= iguana_bundlefind(coin,&bp,&bundlei,block->prev_block)) != 0 )
        {
            ...
            if ( bundlei == coin->chain->bundlesize-1 )
            {
                if ( 1 && coin->bundlescount < bp->hdrsi+1 )
                {
...
struct iguana_bundlereq *iguana_recvblock(struct iguana_info *coin,struct iguana_peer *addr,struct iguana_bundlereq *req,struct iguana_block *origblock,int32_t numtx,int32_t datalen,int32_t recvlen,int32_t *newhwmp)
{
    ...
    if ( block != 0 )
    {
        ...
        if ( bp == 0 && req->copyflag != 0 && block->rawdata == 0 && block->recvlen == 0 )
        {
            ...
            block->copyflag = 1;
        } else block->ipbits = req->ipbits, block->recvlen = recvlen;
    ...
    if ( bp != 0 && bundlei >= 0 )
    {
        ...
        if ( 0 && bp->requests[bundlei] > 2 )
            printf("recv bundlei.%d hdrs.%d reqs.[%d] fpos.%d datalen.%d recvlen.(%d %d) ipbits.(%x %x %x)\n",bundlei,bp->hdrsi,bp->requests[bundlei],bp->fpos[bundlei],datalen,block->recvlen,req->recvlen,block->ipbits,bp->ipbits[bundlei],req->ipbits);
        if ( datalen > 0 )
        {
...
int32_t iguana_pollQsPT(struct iguana_info *coin,struct iguana_peer *addr)
{
        ...
        if ( bestbp != 0 && bp->emitfinish == 0 )
        {
            for (k=0; k<coin->bundlescount; k++)
            {
                ...
                for (r=0; r<m; r++,j++)
                {
                    ...
                    if ( bits256_nonz(hash2) == 0 )
                        continue;
                    if ( bp->requests[j] <= bp->minrequests && bp->fpos[j] < 0 && (j == 0 || bp->issued[j] == 0 || now > bp->issued[j]+val) )
                    {
                        iguana_sendblockreq(coin,addr,bp,j,&hash2,1);
                        return(1);
    ...
    if ( req != 0 )
    {
        hash2 = req->hash2;
        height = req->height;
        if ( priority == 0 && (bp= req->bp) != 0 && req->bundlei >= 0 && req->bundlei < bp->n && req->bundlei < coin->chain->bundlesize && bp->fpos[req->bundlei] >= 0 && req->bundlei != 0 )
        {
            if ( 0 && priority != 0 )
        ...
        else
        {
            ...
            if ( 0 && priority != 0 )
                printf(" issue.%s\n",bits256_str(str,hash2));
            iguana_sendblockreq(coin,addr,req->bp,req->bundlei,&hash2,1);
        }
        flag++;

...
int32_t iguana_processrecv(struct iguana_info *coin) // single threaded
{
    ...
    while ( 1 )
    {
        ...
        if ( (next= iguana_blockfind(coin,iguana_blockhash(coin,coin->blocks.hwmchain.height+1))) != 0 )
        {
            ...
            if ( (coin->blocks.hwmchain.height+1 < coin->longestchain && coin->backstop != coin->blocks.hwmchain.height+1) || milliseconds() > coin->backstopmillis+10*sqrt(coin->blocks.hwmchain.height) )
            {

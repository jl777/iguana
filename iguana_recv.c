...
{
    int32_t i,bundlei,flag = 0; struct iguana_block *block,*prev = 0; struct iguana_bundle *bp;
    ...
    for (i=0; i<num; i++)
    {
        ...
        {
            if ( (block= iguana_blockhashset(coin,-1,blockhashes[i],1)) != 0 && block->ipbits == 0 )
            {

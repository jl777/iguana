...
{
    int32_t i,n = 0; struct iguana_block *block; bits256 hash2;
...
int32_t iguana_issueloop(struct iguana_info *coin)
{
    ...
    int32_t i,bundlei,qsize,m,numwaiting,maxwaiting,numbundles,lastbundle,lastbundle2,n,dispflag = 0,flag = 0;
    ...
    for (i=0; i<coin->bundlescount; i++)
    {
        ...
        if ( (bp= coin->bundles[i]) != 0 )
        {
            ...
            if ( bp->emitfinish == 0 )
            {
                iguana_bundlecheck(coin,bp,numbundles == 0 || i == lastbundle || i == lastbundle2);
                numbundles++;
                if ( numbundles >= coin->MAXBUNDLES && i != lastbundle && i != lastbundle2 )
                    continue;
                for (bundlei=0; bundlei<bp->n && bundlei<coin->chain->bundlesize; bundlei++)
                {
                    ...
                    if ( (i == lastbundle || i == lastbundle2 || numwaiting < maxwaiting) && bits256_nonz(hash2) > 0 )
                    {
        ...
        prevbp = bp;
        if ( dispflag != 0 && (numbundles <= coin->MAXBUNDLES || i == lastbundle || i == lastbundle2) )
